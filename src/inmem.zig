...
pub const IndexBlockHeader = struct {
    ...
    pub fn writeIndexBlock(_: *const IndexBlockHeader, buf: []u8, indexWriter: *Buffer) void {
        // TODO: compress buf zstd or better openzl

        // TODO: write written stats, block window

        indexWriter.write(buf);
    }
};
...
pub const BlockWriter = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !*BlockWriter {
        ...
        const indexWriter = try Buffer.init(allocator);
        w.* = BlockWriter{
            .streamWriter = streamWriter,
            .indexBlockHeader = indexBlockHeader,
            .sidFirst = null,
    ...
    fn flushIndexBlock(self: *BlockWriter) void {
        if (self.indexBlockBuf.len > 0) {
            self.indexBlockHeader.writeIndexBlock(self.indexBlockBuf, self.indexWriter);

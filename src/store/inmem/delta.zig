...
pub fn marshalInt64NearestDelta(packer: anytype, src: []const u64, precisionBits: u8) u64 {
...
fn getTrailingZeros(val: u64, precisionBits: u8) u8 {
    var v = val;
    if (v < 0) {
        v = -v;
...
fn nearestDelta(next: u64, prev: u64, precisionBits: u8, prevTrailingZeros: u8) struct { u64, u8 } {
    const d = next - prev;
    if (d == 0) {
        // Fast path.
